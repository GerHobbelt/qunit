...
(function (global$1) {
  	...
  	run: function run() {
  		...
  		function runTest(test) {
  			...
  			if (test.timeout === 0 && test.semaphore !== 0) {
  				pushFailure("Test did not finish synchronously even though assert.timeout( 0 ) was used.", sourceFromStacktrace(2, true));
  			}
  		}
  	},
  	...
  	queueHook: function queueHook(hook, hookName, hookOwner) {
  		...
  		var runHook = function runHook() {
  			...
  			try {
  				...
  				_this.pushFailure(hookName + " failed on " + _this.testName + ": " + (error.message || error), extractStacktrace(error, 0, true));
  			}
  		};
  	...
  	if (defined.setTimeout) {
  		...
  		if (typeof timeoutDuration === "number" && timeoutDuration > 0) {
  			...
  			config.timeout = setTimeout(function () {
  				pushFailure("Test took longer than " + timeoutDuration + "ms; test timed out.", sourceFromStacktrace(2, true));
  				internalRecover(test);
  			}, timeoutDuration);
  		}
  	}
  ...
  function internalStart(test) {
  	...
  	if (isNaN(test.semaphore)) {
  		...
  		pushFailure("Invalid value on test.semaphore", sourceFromStacktrace(2, true));
  		return;
  	}
  	...
  	if (test.semaphore < 0) {
  		...
  		pushFailure("Tried to restart test while already started (test's semaphore was 0 already)", sourceFromStacktrace(2, true));
  		return;
  	}
